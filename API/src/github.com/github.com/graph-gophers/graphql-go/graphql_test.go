...
func (r *timeResolver) AddHour(args struct{ Time Time }) Time {
	return Time{Time: args.Time.Add(time.Hour)}
}
...
var starwarsSchema = MustParseSchema(starwars.Schema, &starwars.Resolver{})
...
func TestRootOperations_invalidSchema(t *testing.T) {
	...
	for name, tt := range testTable {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, err := ParseSchema(tt.Args.Schema, nil)
	...
	gqltesting.RunTests(t, []*gqltesting.Test{
		{
			...
			`, &testEmbeddedStructResolver{}, UseFieldResolvers()),
			Query: `
				{
					course{
						name
...
var starwarsSchemaNoIntrospection = MustParseSchema(starwars.Schema, &starwars.Resolver{}, []SchemaOpt{DisableIntrospection()}...)
...
func (r *inputResolver) ID(args struct{ Value ID }) ID {
	return args.Value
}
...
func TestInput(t *testing.T) {
	...
	coercionSchema := MustParseSchema(`
		schema {
			query: Query
		}
...
func TestInputArguments_failSchemaParsing(t *testing.T) {
	...
	for name, tt := range testTable {
		...
		t.Run(name, func(t *testing.T) {
			...
			_, err := ParseSchema(tt.Args.Schema, tt.Args.Resolver)
			if err == nil || err.Error() != tt.Want.Error {
...
func TestPanicAmbiguity(t *testing.T) {
	...
	`
	MustParseSchema(schema, &ambiguousResolver{}, UseFieldResolvers())
}
...
func TestSchema_Exec_without_resolver(t *testing.T) {
	...
	for _, tt := range testTable {
		t.Run(tt.Name, func(t *testing.T) {
			s := MustParseSchema(tt.Args.Schema, nil)
